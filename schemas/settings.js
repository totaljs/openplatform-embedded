...
NEWSCHEMA('Settings', function(schema) {
	...
	schema.setRead(function($) {
		...
		DBMS().find('config').fields('id,type,value').callback(function(err, response) {
	...
	schema.setSave(function($) {
		...
		for (var i = 0; i < keys.length; i++) {
			var key = keys[i];
			var val = model[key] + '';
			db.modify('config', { value: val, dtupdated: NOW }, true).where('id', key).insert(insert_config, { id: key, type: typeof(model[key]) });
		}
